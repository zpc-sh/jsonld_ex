defmodule JsonldEx.Diff.Performance do
  ...
  defp compute_lcs_elixir(old_array, new_array, _opts) do
    ...
    operations =
      extract_lcs_operations(lcs_table, old_array, new_array, length(old_array), length(new_array))
    ...
    Enum.reduce(1..m, table, fn i, acc_table ->
      Enum.reduce(1..n, acc_table, fn j, inner_acc ->
        ...
        if old_val == new_val do
          put_in(
            inner_acc,
            [Access.at(i), Access.at(j)],
            get_in(inner_acc, [Access.at(i - 1), Access.at(j - 1)]) + 1
          )
        else
          max_val =
            max(
              get_in(inner_acc, [Access.at(i - 1), Access.at(j)]),
              get_in(inner_acc, [Access.at(i), Access.at(j - 1)])
            )
