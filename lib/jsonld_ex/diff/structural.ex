defmodule JsonldEx.Diff.Structural do
  ...
  # [old, new] for changes, [old, 0, 0] for deletions, ["", from, 3] for moves, [text_diff, 0, 2] for text
  @type delta_value ::
          list(term())
          # nested object diffs
          | map()
  ...
  defp detect_array_moves(old, new, opts) do
    ...
    old_with_index =
      old |> Enum.with_index() |> Enum.map(fn {val, idx} -> {hash_value(val), {val, idx}} end)

    new_with_index =
      new |> Enum.with_index() |> Enum.map(fn {val, idx} -> {hash_value(val), {val, idx}} end)
  ...
  defp patch_array(document, delta, opts) when is_list(document) do
    ...
    deletes =
      operations
      |> Enum.filter(fn {op, _, _} -> op == :delete end)
    ...
    # Apply deletes first
    result_after_deletes =
      Enum.reduce(deletes, document, fn {:delete, index, _}, acc ->
    ...
    Enum.reduce(others, result_after_deletes, fn operation, acc ->
      case operation do
        {:insert, index, [new_value]} ->
          if index <= length(acc),
            do: List.insert_at(acc, index, new_value),
            else: acc ++ [new_value]
  ...
  defp apply_text_diff_ops(text, ops) when is_binary(text) and is_list(ops) do
    ...
    try do
      ...
      final_cursor =
        Enum.reduce(ops, 0, fn op, pos_old ->
          case op do
