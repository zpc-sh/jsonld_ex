defmodule JsonldEx.C14n do
    ...
    with {:miss, nil} <- {:miss, cache_get(key)},
      ...
      case result do
        {:ok, nquads} when is_binary(nquads) ->
          ...
          :telemetry.execute([:jsonld, :canonicalize, :stop], %{}, %{
            algorithm: alg,
            provider: provider,
            bytes: byte_size(nquads)
          })

          {:ok, %{nquads: nquads, bnode_map: %{}}}

        {:error, reason} ->
          :telemetry.execute([:jsonld, :canonicalize, :error], %{}, %{
            provider: provider,
            reason: reason
          })
  ...
  def hash(term, opts \\ []) do
    ...
    data =
      case form do
        ...
        other ->
          raise ArgumentError, "unsupported form: #{inspect(other)}"
      end
  ...
  defp count_lines(bin) when is_binary(bin),
  ...
  defp provider do
    case System.get_env("JSONLD_CANON_PROVIDER") do
      ...
      s ->
        String.to_atom(String.downcase(s))
    end
  rescue
    _ -> :none
  end
