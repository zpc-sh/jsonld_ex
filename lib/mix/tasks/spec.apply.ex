defmodule Mix.Tasks.Spec.Apply do
  ...
  def run(argv) do
    ...
    messages =
      Path.wildcard(Path.join(src_dir, "msg_*.json"))
      ...
      if messages == [] do
        ...
        Enum.reduce(messages, [], fn {msg_path, msg}, acc ->
          ...
          |> Enum.reduce(acc, fn rel, acc2 ->
            ...
            info =
              case result do
                {:ok, info} when dry_run ->
                  Mix.shell().info(
                    "[DRY-RUN] Would apply #{length(info.paths)} ops to #{info.file} (baseline_ok=#{info.baseline_ok}, baseline_git_ok=#{info.baseline_git_ok})"
                  )
                ...
                {:ok, info} ->
                  Mix.shell().info(
                    "Applied #{length(info.paths)} ops to #{info.file} (baseline_ok=#{info.baseline_ok}, baseline_git_ok=#{info.baseline_git_ok}) from #{rel} (msg #{Path.basename(msg_path)})"
                  )
            ...
            acc2 ++
              [Map.merge(info, %{message: Path.basename(msg_path), patch: rel, dry_run: dry_run})]
          end)
        end)
      end
  ...
  defp apply_patch!(
    ...
    file_rel =
      patch["file"] || get_in(msg, ["ref", "path"]) ||
        Mix.raise("Patch missing file and message.ref.path")
    ...
    if (not baseline_ok or not baseline_git_ok or not baseline_git_hash_ok) and not force do
      Mix.raise(
        "Baseline mismatch for #{file_rel} (hash_ok=#{baseline_ok}, git_eq=#{baseline_git_ok}, git_hash=#{baseline_git_hash_ok}). Use --force to override."
      )
    end
    ...
    {final, applied_paths} =
      cond do
        is_list(ops) ->
          Enum.reduce(ops, {json, []}, fn op, {acc, paths} ->
            ...
            case op_name do
              ...
              "remove" ->
                if not pointer_exists?(acc, ptr),
                  do: Mix.raise("Pointer not found for remove: #{ptr}")
            ...
            effective_op =
              cond do
                op_name == "replace" and pointer_exists?(acc, ptr) and not allow_type_change? ->
                  case get_in_pointer(acc, pointer_tokens(ptr)) do
                    {:ok, current} ->
                      ...
                      if type_tag(current) == type_tag(v),
                        do: op,
                        else:
                          Mix.raise(
                            "Type change not allowed at #{ptr}: #{type_tag(current)} -> #{type_tag(v)}"
                          )
                ...
                op_name == "replace" and not pointer_exists?(acc, ptr) and replace_create? ->
                  Map.put(op, "op", "add")
        ...
        true ->
          Mix.raise("Patch missing ops or merge document")
      end
    ...
    info = %{
      ...
      baseline_git_ok: baseline_git_ok,
      baseline_git_hash_ok: baseline_git_hash_ok,
      paths: applied_paths,
      before: before_str,
      after: after_str,
      id: id
  ...
  defp apply_op!(doc, base, %{"op" => op, "path" => path} = o) do
    ...
    case op do
      "add" ->
        json_put(doc, ptr, Map.fetch!(o, "value"), :add)

      "replace" ->
        json_put(doc, ptr, Map.fetch!(o, "value"), :replace)
      ...
      "copy" ->
        ...
        case get_in_pointer(doc, pointer_tokens(from)) do
          ...
          :error ->
            Mix.raise("Pointer not found for copy.from: #{from}")
        end
      ...
      "move" ->
        ...
        case get_in_pointer(doc, pointer_tokens(from)) do
          ...
          :error ->
            Mix.raise("Pointer not found for move.from: #{from}")
        end

      other ->
        Mix.raise("Unsupported op: #{inspect(other)}")
    end
  end
  ...
  defp put_in_pointer(_other, _rest, _val, _mode),
    do: Mix.raise("Unsupported structure for pointer")

  defp remove_in_pointer(doc, [{:key, k}]) when is_map(doc), do: Map.delete(doc, k)

  defp remove_in_pointer(list, [{:idx, i}]) when is_list(list) and i < length(list),
  ...
  defp git_show_file(dir, rev, rel_path) do
    if File.dir?(Path.join(dir, ".git")) do
      {out, status} =
        System.cmd("git", ["-C", dir, "show", rev <> ":" <> rel_path], stderr_to_stdout: true)
  ...
  defp write_temp_and_prepare_diff(info) do
    ...
    {out, _} =
      System.cmd("git", ["--no-pager", "diff", "--no-index", "--no-color", before_path, after_path],
